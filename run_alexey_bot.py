#!/usr/bin/env python3
"""Standalone entry point for the CEO Alexey Telegram bot with retry."""
...
async def run_with_retry():
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            print(f"[Алексей bot] Starting (attempt {attempt}/{MAX_RETRIES})...", flush=True)
            await main()
            break
            ...
            if attempt < MAX_RETRIES:
                ...
                print(f"[Алексей bot] Max retries reached, giving up.", flush=True)
                sys.exit(1)
